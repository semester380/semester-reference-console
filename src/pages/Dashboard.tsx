...
const Dashboard: React.FC = () => {
    ...
    const loadRequests = React.useCallback(async () => {
        ...
        try {
            ...
            const response = await runGAS('getMyRequests') as any;
    ...
    const calculateStats = (data: Request[]) => {
        setStats({
            total: data.length,
            pending: data.filter(r => r.status === 'PENDING_CONSENT' || r.status === 'Sent' || r.status === 'Pending_Consent').length,
            completed: data.filter(r => ['Completed', 'Consent_Given', 'CONSENT_GIVEN', 'SEALED', 'Declined'].includes(r.status)).length,
            flagged: data.filter(r => r.anomalyFlag || r.status === 'EXPIRED' || r.status === 'Flagged').length,
        });
    };
        ...
        <div className="min-h-screen bg-nano-gray-50 font-sans">
            ...
            {showToast && (
                <div className="fixed top-4 right-4 z-50 animate-slide-in">
                    <div className="bg-status-success text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-2">
            ...
            <main className="max-w-7xl mx-auto px-6 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setStatusFilter('all')}>
                        <div className="text-sm text-nano-gray-600 mb-1">Total Requests</div>
